...
def build_json_graph(repo_path, fuzzy_search=True, global_import=False):
    ...
    for edge in edges:
        ...
        if from_node not in edge_dic:
            edge_dic[from_node] = []
        edge_dic[from_node].append(edge)
    for node in nodes:
        node_name = node['id']
        node_type = node['type']
        if (node_type == NODE_TYPE_FILE) or (node_type == NODE_TYPE_DIRECTORY):
            if node_name not in folders:
                dep_idc = {k: [] for k in VALID_EDGE_TYPES}
                folders[node_name] = {
                    'name': node_name,
                    'type': node_type,
                    'deps': dep_idc
                }
            if node_name in edge_dic:
                for edge in edge_dic[node_name]:
                    dep_type = edge['type']
                    if dep_type != EDGE_TYPE_IMPORTS:
                        target = edge['target']
                        if target not in folders[node_name]['deps'][dep_type]:
                            folders[node_name]['deps'][dep_type].append(target)
                    else:
                        target = {
                            'name': edge['target'],
                            'alias': edge['alias']
            ...
            if node_name in edge_dic:
                for edge in edge_dic[node_name]:
                    ...
                    if dep_type != EDGE_TYPE_IMPORTS:
                        target = edge['target']
                        if target not in codes[node_name]['deps'][dep_type]:
                            codes[node_name]['deps'][dep_type].append(target)
                    else:
                        target = {
                            'name': edge['target'],
                            'alias': edge['alias']
...
def visualize_graph(G, output_file='plots/dp_v3.png'):
